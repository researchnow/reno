...
var RenoTablePager = function (_HTMLElement) {
	...
	function RenoTablePager(self) {
		var _this, _ret;

		_classCallCheck(this, RenoTablePager);

		self = (_this = _possibleConstructorReturn(this, (RenoTablePager.__proto__ || Object.getPrototypeOf(RenoTablePager)).call(this, self)), _this);
			...
			for (; node && node !== this; node = node.parentNode) {
				...
				if (reason) {
					...
					if (offset && !node.classList.contains('ignore') && (this.getAttribute('current') !== 'ignore' || offset !== this.getAttribute('offset'))) {
						this.dispatchEvent(new CustomEvent('page-selected', { bubbles: true, detail: { reason: reason, offset: offset } }));
						return;
					}
				}
			}
