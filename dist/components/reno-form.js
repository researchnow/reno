...
(function () {
  ...
  var RenoForm = function (_HTMLElement) {
    ...
    _createClass(RenoForm, [{
      ...
      key: 'getElements',
      value: function getElements(clean, skipButtons) {
        var elements = [],
        ...
        for (var i = 0; i < forms.length; ++i) {
          ...
          for (var j = 0; j < form.length; ++j) {
            ...
            if (clean && tag === 'input' && (type === 'radio' || type === 'checkbox') ? !element.checked : false) continue;
      ...
      value: function getElementsByName(clean, skipButtons) {
        var _this3 = this;

        var dict = {};
        this.getElements(clean, skipButtons).forEach(function (element) {
      ...
      key: 'getValues',
      value: function getValues(skipButtons) {
        var _this4 = this;

        var form = {};
        this.getElements(true, skipButtons).forEach(function (element) {
          ...
          if (form.hasOwnProperty(element.name)) {
            switch (element.type && element.type.toLowerCase()) {
              case 'radio':
                ...
                break;
              case 'checkbox':
                var values = form[element.name];
                if (values) {
                  element.checked = values.some(function (value) {
                    _newArrowCheck(this, _this5);
