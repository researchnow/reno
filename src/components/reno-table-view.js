(function() {
  ...
  const renderValue = value =>
    primitives[typeof value] ? {html: value} : typeof value == 'function' ? value() : value;
  ...
  class RenoTableView extends HTMLElement {
    ...
    io() {
      ...
      heya
        .io(this.sanitizeRequest({url: url, method: 'GET', query: request}))
        .then(page => {
          ...
          this.render();
          this.dispatchEvent(
            new CustomEvent('reno-table-data-updated', {
              bubbles: true,
              detail: {limit, total: this.total, offset: this.realOffset, shown: this.realLimit}
            })
          );
    ...
    onClick(e) {
      ...
      for (; node && node !== this; node = node.parentNode) {
        if (
          node.classList.contains('ignore-click') ||
          (activeElements[node.tagName.toLowerCase()] === 1 && !node.classList.contains('include-click'))
        )
          break;
          ...
          if (parent.classList.contains('tbody')) {
            ...
            for (let i = 0; i < parent.childNodes.length; ++i) {
              if (parent.childNodes[i] === node) {
                this.dispatchEvent(
                  new CustomEvent('reno-table-item-selected', {bubbles: true, detail: {item: this.page.data[i]}})
                );
                return;
              }
            }
