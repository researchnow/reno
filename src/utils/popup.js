...
function alignAtTop(popupContainer, popupContent, heightMargin) {
  // when content is too long - move to top of page
  popupContainer.style.top = window.pageYOffset + heightMargin + 'px';
  popupContent.style.maxHeight = window.innerHeight - heightMargin * 2 + 'px';
}
...
function calculatePlacement(popupComponent, popupContainer, options) {
  ...
  let popupContent = document.querySelector('#reno-popup-container .content.list.domain__headerActions--versionsList');
    ...
    case 'left':
      if (popupComponentDomRect.left - popupContainerDomRect.width - popupContainerMargin < 0) {
        ...
        if (window.innerWidth - popupComponentDomRect.right > popupComponentDomRect.left) {
          ...
          popupContainer.style.maxWidth = window.innerWidth - popupComponentDomRect.right - widthMargin + 'px';
        } else {
          // there is more room on the left, so put popup there
          popupContainer.style.left = widthMargin + 'px';
          popupContainer.style.maxWidth = popupComponentDomRect.left - widthMargin + 'px';
        }
      } else {
        ...
        if (popupComponentDomRect.top < window.innerHeight - popupComponentDomRect.bottom) {
          ...
          if (neededSpace > availableSpace) {
            ...
            if (neededSpace > window.innerHeight) {
              ...
              alignAtTop(popupContainer, popupContent, heightMargin);
            } else {
              // container is too big, but not bigger than window, so set max height
              popupContent.style.maxHeight = window.innerHeight - popupComponentDomRect.bottom - heightMargin + 'px';
          ...
          if (popupContainerDomRect.height + popupContainerMargin > window.innerHeight) {
            ...
            // container is too big, but not bigger than window, so set max height
            popupContainer.style.top = window.pageYOffset + heightMargin + 'px';
            popupContent.style.maxHeight = popupComponentDomRect.top - heightMargin - 2 + 'px';
          }
        }
      } else {
        // plenty of room, proceed as planned
  ...
  switch (alignment) {
    case 'left':
      ...
      if (popupContainerDomRect.width + popupContainerMargin > window.innerWidth - popupComponentDomRect.left) {
        ...
        popupContainer.style.maxWidth = window.innerWidth - widthMargin * 2 + 'px';
      }
    ...
    case 'right':
      if (popupContainerDomRect.width + popupContainerMargin > popupComponentDomRect.right) {
        ...
        popupContainer.style.left = widthMargin + 'px';
        popupContainer.style.maxWidth = anchor - widthMargin + 'px';
      } else {
    ...
    case 'top':
      if (popupContainerDomRect.height + popupContainerMargin > window.innerHeight) {
        ...
        popupContent.style.maxHeight = window.innerHeight - popupComponentDomRect.top - heightMargin + 'px';
      }
    ...
    case 'bottom':
      if (popupContainerDomRect.height + popupContainerMargin > window.innerHeight) {
        ...
        popupContent.style.maxHeight = popupComponentDomRect.bottom - heightMargin + 'px';
      }
      break;
    default:
      // case 'center':
